...
Keluaran dari bagian pembuatan \textit{dataset} ini adalah sebuah berkas berisi CCG \textit{supertag}
lengkap dan beberapa berkas CCG \textit{supertag} yang lemanya dikelompokkan berasarkan alfabet.
Adapun format \textit{dataset} yang disimpan dalam berkas tersebut dapat dilihat pada Gambar
\ref{dataset:format}.
Untuk setiap kalimat dalam berkas tersebut direpresentasikan oleh dua baris yaitu
(1) daftar token yang dipisahkan oleh spasi, dan
(2) daftar \textit{supertag} yang dibungkus kurung siku (contoh: $\text{[\textit{supertag}]}$) dan
dipisahkan oleh spasi sejajar dengan tokennya.
...
\begin{figure}\centering
	\bgroup
	\tabbedShortstack{
		$\text{token}_1$ & $\text{token}_2$ & $\text{token}_3$ & $\dots$ & $\text{token}_n$\\
		$\text{supertag}_1$ &
			$\text{supertag}_2$ & $\text{supertag}_3$ & $\dots$ & $\text{supertag}_n$
	}
	\egroup
	\caption{Format \textit{dataset} yang disimpan di dalam berkas.}
...
\begin{figure}\centering\tiny
	...
	\tabbedShortstack{
		Pamungkas & dan & Setyo & menyukai & rendang & .\\
		$\text{[NP\ :\ \so{pamungkas}]}$ &
			$\text{[CONJ\ :\ $\lambda x.\lambda y.\lambda f.\ (f\ x) \land (f\ y)$]}$ &
			$\text{[NP\ :\ \so{Setyo}]}$ &
			$\text{[(S{$\backslash$}NP)/NP\ :\ $\lambda x.\lambda y.\ suka(y, x)$]}$ &
			$\text{[NP\ :\ \so{rendang}]}$ &
			$\text{[Z]}$
	}
	\egroup
	\caption{Contoh isi dari berkas \textit{dataset}.}
