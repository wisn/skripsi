...
\begin{figure}\centering\small
  ...
  \begin{tikzpicture}[node distance=1.5cm, every edge/.style={link}]
    ...
    \node[attribute] (acc-uuid) [above=of acc] {\key{UUID}} edge (acc);
    \node[attribute] (acc-email) [above right=of acc] {Email} edge (acc);
    \node[attribute] (acc-password) [right=of acc] {Password} edge (acc);
